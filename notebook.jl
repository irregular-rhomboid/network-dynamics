...
# ╔═╡ 29d785b8-fb3d-11ed-337f-6b1177c4ff3e
begin
...
# ╔═╡ a2b380de-4436-4e82-8615-befb19e98f88
brusselator_x(x,y,a,b) = a + x^2 * y - b*x - x

# ╔═╡ 86d60531-663b-4872-bbea-7c6747e1f1ff
brusselator_y(x,y,a,b) = b*x - x^2 * y

# ╔═╡ 9d351785-88d9-4da3-8844-f4bdd4924c89
function brusselator!(dx,x,p,t)
	dx[1] = brusselator_x(x[1], x[2], p[1],  p[2])
	dx[2] = brusselator_y(x[1], x[2], p[1],  p[2])
end

# ╔═╡ 89effb4a-944f-4699-ac5c-8b840d1e7110
N = 5

# ╔═╡ ae6932f1-bda4-49c1-8157-91606460e0ab
@parameters t a b L[1:N,1:N] D_u D_v

# ╔═╡ ca9be469-bce1-40e0-958a-a1cde23c6527
@variables u(t)[1:N] v(t)[1:N]

# ╔═╡ 4987e7d9-f1fd-4b6f-806b-a1b883e72e4f
scalarize(u)

# ╔═╡ 060cc986-0d98-41e5-b76c-4a207b7b4617
dudt = brusselator_x.(u,v,a,b) - D_u * (L * u)

# ╔═╡ ce339867-b41f-4fc2-9590-a91d44f656d1
dvdt = brusselator_y.(u,v,a,b) - D_v * (L * v)

# ╔═╡ ed659da9-386f-4f81-ac99-b6697b023a78
D = Differential(t)

# ╔═╡ 6cc25128-5bbe-450e-b0c1-3e1ed1923b66
eqs_1 = [
	D.(u) ~ dudt;
	D.(v) ~ dvdt
] #.|> scalarize

# ╔═╡ bb4e894b-a553-4724-adf4-a7c2942987a2
@named sys_1 = ODESystem(eqs_1)

# ╔═╡ 1d8fc559-1421-4bce-b0b0-4a89044421d3
eqs = [
	D.(scalarize(u)) .~ scalarize(dudt);
	D.(scalarize(v)) .~ scalarize(dvdt)
]

# ╔═╡ e829b335-a2d2-49b6-a69a-f4d555127633
@named sys = ODESystem(expand_derivatives.(eqs))

# ╔═╡ 4049c0b7-582f-4623-8b72-8cbca82ccd62
begin
	g = watts_strogatz(N, 3, 0.4)
	L_ = laplacian_matrix(g)
end

# ╔═╡ 25169f4f-3614-4f12-b136-0fbd3c8f67d2
p = [
	a => 1.0,
	b => 3.0,
	collect(L .=> L_)...,
...
# ╔═╡ 0eedf8aa-3358-44e6-b6ff-56b093369041
u0 = [
	collect(u .=> rand(N));
	collect(v .=> rand(N))
]

# ╔═╡ fa12a4ab-35bc-4fc6-8325-016984cd7dd6
prob = ODEProblem(sys, u0, (0.0,20.0), p)

# ╔═╡ 214346d8-1e56-4e6a-a374-49f647877fe6
sol = solve(prob, Rodas5())

# ╔═╡ b208b0c3-d415-49e4-81c1-9c4066da40d6
plot(sol)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "545f7da29337c324094aeaf224bd23c724f20d07"
